...
由于 `k8s/base/secret.yaml` 中是占位符，直接 apply 会导致应用启动失败。本地验证时，使用真实或测试密钥创建 Secret。
...
预期输出：创建 namespace、configmap、secret、serviceaccount、service、deployment、statefulset、networkpolicy、ingress 等资源。
...
1. Preflight / Quality / Security / Unit / Red Team / Frontend 检查
2. 构建并推送镜像
3. 部署到 Staging
4. 等待 rollout
5. 执行 `/health` 冒烟测试
...
在 GitHub Actions 中查看 `deploy-staging` job 输出，确认：
