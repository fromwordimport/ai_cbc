...
- [ ] GitHub 仓库已创建：`https://github.com/fromwordimport/ai_cbc`
- [ ] 代码已 push 到 `master` 分支
...
Render 会为每个 service 创建后，在 Dashboard 中填入以下 secret 值：

| Service | 变量名 | 来源 |
|---------|--------|------|
| aicbc-api / aicbc-worker / aicbc-beat | `MONGODB_URL` | MongoDB Atlas 连接串 |
| aicbc-api / aicbc-worker / aicbc-beat | `REDIS_URL` | Upstash Redis URL |
| aicbc-api / aicbc-worker / aicbc-beat | `ANTHROPIC_API_KEY` | Anthropic Console |
| aicbc-api / aicbc-worker / aicbc-beat | `OPENAI_API_KEY` | OpenAI Platform |
| aicbc-api | `SECRET_KEY` | Render 自动生成，无需修改 |

> **注意**：`SECRET_KEY` 由 Render 在 `aicbc-api` 中自动生成，`aicbc-worker` 和 `aicbc-beat` 会从 `aicbc-api` 同步。
...
3. 测试健康检查：
   ...
   ```
4. 检查 aicbc-worker 日志，确认 Celery 已连接 Redis
...
| 项目 | 说明 |
|------|------|
| **免费 Web Service 休眠** | Render free web service 15 分钟无访问会休眠，下次请求唤醒约 30 秒 |
...
| **大样本分析** | Render free worker 内存有限，大样本 HB 分析可能 OOM，需降低并发 |
...
render logs --service aicbc-worker
render logs --service aicbc-beat
```

或在 Render Dashboard → Service → Logs 中查看。
