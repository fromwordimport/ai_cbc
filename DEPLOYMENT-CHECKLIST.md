...
> **重要**：Render Static Site 与 Cloudflare 橙色云代理冲突，会导致 error 1000。前端子域名必须设置为 **DNS only（灰色云）**。（已复核 2026-06-17，参考 Render 官方文档 https://render.com/docs/cloudflare）
...
| **免费 Web Service 休眠** | Render free web service 15 分钟无访问会休眠，下次请求唤醒约 30 秒；休眠期间 Celery Worker 和 Celery Beat 也停止运行。此方案仅适合个人使用，不适合需要可靠后台任务执行的生产环境。 |
