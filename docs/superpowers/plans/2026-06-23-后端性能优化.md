...
> **For agentic workers：** 本计划已执行完毕，如需复用请先查看下方「执行记录」。
...
| 工作分支 | `perf/backend-optimization-2026-06-25` |
| 合并提交 | `c0696ab` |
| 合并日期 | 2026-06-25 |
| CI 状态 | ✅ 通过（run `28183887395`） |
| CD 状态 | ✅ Azure B2ats v2 通过（run `28183887325`） |
| 实施范围 | 全部 24 个任务已完成 |
...
- [x] **Step 1: 编写 profile_endpoint.py**
...
- [x] **Step 2: 编写 benchmark_image.py**
...
- [x] **Step 2: 修改 prometheus.yml 挂载 rules**
...
- [x] **Step 3: 在 docker-compose.yml 中挂载 rules**
...
- [x] **Step 2: 将告警规则合并到 prometheus-rules.yml**
...
- [x] **Step 3: 在 docker-compose.yml 中添加 alertmanager 服务**
...
- [x] **Step 1: 添加新面板到 dashboard JSON**
...
- [x] **Step 1: 在 MongoQuestionnaireStore 中添加聚合方法**
...
- [x] **Step 2: 修改 dashboard_summary 函数**
...
- [x] **Step 1: 添加 PersonaGenerationJobDocument**
...
- [x] **Step 2: 更新 ALL_DOCUMENT_MODELS**
...
- [x] **Step 2: 删除原 generate_batch 中重复的结果组装代码**
...
- [x] **Step 1: 添加 AsyncClient 与异步调用方法**
...
- [x] **Step 2: 新增异步 Anthropic/OpenAI 调用方法**
...
- [x] **Step 3: 修改 generate_async 使用异步路径**
...
- [x] **Step 2: 更新 docker-compose.yml 中 SLOW_REQUEST_THRESHOLD**
...
- [x] **Step 1: 在 builder stage 后添加 api target**
...
- [x] **Step 1: 补全 docker-compose.yml 资源限制**
...
- [x] **Step 3: 更新 k8s/base/configmap.yaml**
...
- [x] **Step 1: 在 lifespan 中注册 SIGTERM 处理**
...
- [x] **Step 2: 在 RateLimitMiddleware 前增加关闭检测**
...
- [x] **Step 2: 在 CI 中新增 performance-baseline job**
...
本计划已通过 Subagent-Driven Development 执行完毕，全部 24 个任务已完成并合并至 `master`（合并提交 `c0696ab`）。

- CI 验证：通过（run `28183887395`）
- CD 验证：Azure B2ats v2 通过（run `28183887325`）
- 性能基线实测数据、火焰图与内存图待补充至 `reports/performance/2026-06-23-baseline.md`。
