...
from aicbc.core.models.persona import (
    DishwasherContext, GenerationMetadata, Layer1Demographics, Layer2Behavior,
    Layer3Psychology, Layer4Scenarios, PersonaProfile, TensionCombination,
)
...
def make(gender, city, occupation, income, price_sens, decision, samples=None):
    return PersonaProfile(
        ...
        layer1_demographics=Layer1Demographics(
            age="28岁", gender=gender, city=city, income=income,
            occupation=occupation, education="本科",
            marital_status="已婚", living_type="自有住房",
        ),
        layer2_behavior=Layer2Behavior(
            price_sensitivity=price_sens, purchase_channels=[],
            decision_style=decision, brand_loyalty="中等", information_source=[],
        ),
        layer3_psychology=Layer3Psychology(
            core_values=[], core_anxieties=[],
            ...
            secret_motivation="品味证明", defense_mechanism="合理化",
        ),
        layer4_scenarios=Layer4Scenarios(
            daily_routine="早7点起床", purchase_trigger="被小红书种草",
            stress_response="焦虑时刷购物APP", social_behavior="朋友圈少发",
        ),
        ...
        dishwasher_context=DishwasherContext(
            purchase_constraints=[], decision_factors=[], ignored_factors=[],
        ),
...
def test_student_high_income_fails():
    ...
    assert len(occ) >= 1, f"Expected occupation-income finding, got {[f.rule_id for f in r.findings]}"
    ...
    print(f"  [OK] Student+100w: status={r.status}, findings={len(r.findings)}, high={r.high_severity_count}")


def test_female_emotional_stereotype():
    """Female + emotional keywords should trigger gender stereotype patterns."""
    auditor = BiasAuditor()
    p = make("女", "新一线", "白领", "15-30万元",
             "感性消费，冲动消费，情绪化决策，容易被种草",
             "感性冲动，只看颜值")
    r = auditor.audit(p)
    gender = [f for f in r.findings if f.category == "gender"]
    assert len(gender) >= 1, f"Expected gender findings, got none. Findings: {[f.rule_id for f in r.findings]}"
...
def test_language_bias():
    ...
    auditor = BiasAuditor()
    p = make("女", "新一线", "白领", "15-30万元", "中等敏感", "理性比较型",
             samples=[
...
def test_clean_persona_passes():
    ...
    auditor = BiasAuditor()
    p = make("女", "二线城市", "教师", "8-15万元",
             "比较理性，注重性价比", "口碑参考型")
    r = auditor.audit(p)
    # May have diversity flag (low severity), but should not FAIL
    assert r.status == "PASSED", f"Expected PASSED, got {r.status}: {[f.rule_id for f in r.findings]}"
...
def test_24_patterns_categories():
    ...
    cats = set(p["category"] for p in STEREOTYPE_PATTERNS)
